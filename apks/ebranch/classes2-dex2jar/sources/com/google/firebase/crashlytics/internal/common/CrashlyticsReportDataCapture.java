...
import o.onClickNo;
/* loaded from: classes2-dex2jar.jar:com/google/firebase/crashlytics/internal/common/CrashlyticsReportDataCapture.class */
public class CrashlyticsReportDataCapture {
    ...
    private CrashlyticsReport.Session.Event.Device populateEventDeviceData(int i) {
        Float f;
        onClickNo d = onClickNo.d(this.context);
        Float f2 = d.b;
        return CrashlyticsReport.Session.Event.Device.builder().setBatteryLevel(f2 != null ? Double.valueOf(f2.doubleValue()) : null).setBatteryVelocity((!d.d || (f = d.b) == null) ? 1 : ((double) f.floatValue()) < 0.99d ? 2 : 3).setProximityOn(CommonUtils.getProximitySensorEnabled(this.context)).setOrientation(i).setRamUsed(CommonUtils.getTotalRamInBytes() - CommonUtils.calculateFreeRamInBytes(this.context)).setDiskUsed(CommonUtils.calculateUsedDiskSpaceInBytes(Environment.getDataDirectory().getPath())).build();
    }
    ...
    private CrashlyticsReport.Session.Device populateSessionDeviceData() {
        ...
        return CrashlyticsReport.Session.Device.builder().setArch(deviceArchitecture).setModel(Build.MODEL).setCores(availableProcessors).setRam(totalRamInBytes).setDiskSpace(blockCount * blockSize).setSimulator(isEmulator).setState(deviceState).setManufacturer(str).setModelClass(Build.PRODUCT).build();
    }
