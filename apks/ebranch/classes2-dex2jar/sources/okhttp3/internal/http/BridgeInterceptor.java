...
public final class BridgeInterceptor implements Interceptor {
    ...
    public final Response intercept(Interceptor$Chain interceptor$Chain) throws IOException {
        ...
        if (z && "gzip".equalsIgnoreCase(proceed.header("Content-Encoding")) && HttpHeaders.hasBody(proceed)) {
            ChuckerDatabase_Impl chuckerDatabase_Impl = new ChuckerDatabase_Impl(proceed.body().source());
            request2.headers(proceed.headers().newBuilder().removeAll("Content-Encoding").removeAll("Content-Length").build());
            request2.body(new RealResponseBody(proceed.header("Content-Type"), -1, ChuckerDatabase.b((defaultInspectorModulesProvider) chuckerDatabase_Impl)));
        }
