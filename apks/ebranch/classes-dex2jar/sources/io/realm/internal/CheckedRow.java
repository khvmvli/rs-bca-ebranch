...
/* loaded from: classes-dex2jar.jar:io/realm/internal/CheckedRow.class */
public class CheckedRow extends UncheckedRow {
    ...
    public Row freeze(OsSharedRealm osSharedRealm) {
        return !isValid() ? InvalidRow.INSTANCE : new CheckedRow(this.context, this.parent.freeze(osSharedRealm), nativeFreeze(getNativePtr(), osSharedRealm.getNativePtr()));
    }

    @Override // io.realm.internal.UncheckedRow, io.realm.internal.Row
    public OsList getModelList(long j) {
        if (getTable().getColumnType(j) == RealmFieldType.LIST) {
            return getModelList(j);
        }
    ...
    public OsList getValueList(long j, RealmFieldType realmFieldType) {
        if (realmFieldType == getTable().getColumnType(j)) {
            return getValueList(j, realmFieldType);
        }
    ...
    public OsMap getValueMap(long j, RealmFieldType realmFieldType) {
        if (realmFieldType == getTable().getColumnType(j)) {
            return getValueMap(j, realmFieldType);
        }
        throw new IllegalArgumentException(String.format(Locale.US, "The type of field '%1$s' is not 'RealmFieldType.%2$s'.", getTable().getColumnName(j), realmFieldType.name()));
    }
    ...
    public OsSet getValueSet(long j, RealmFieldType realmFieldType) {
        if (realmFieldType == getTable().getColumnType(j)) {
            return getValueSet(j, realmFieldType);
        }
        throw new IllegalArgumentException(String.format(Locale.US, "The type of field '%1$s' is not 'RealmFieldType.%2$s'.", getTable().getColumnName(j), realmFieldType.name()));
    }
