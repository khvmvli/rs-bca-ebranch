...
@Parcel
/* loaded from: classes-dex2jar.jar:com/bca/smartbranch/data/api/response/GetTxnTellerDetailKUResponse.class */
public class GetTxnTellerDetailKUResponse {
    @infoContactPersonDialogEvent(c = "txn_data")
    private List<TxnDetail> txnDetail;

    @Parcel
    /* loaded from: classes-dex2jar.jar:com/bca/smartbranch/data/api/response/GetTxnTellerDetailKUResponse$TxnDetail.class */
    public static class TxnDetail {
        ...
        @infoContactPersonDialogEvent(c = "sender_address_2")
    ...
    public List<TxnDetail> getTxnDetailsByNoreff(String str) {
        String substring = str.substring(0, 3);
        ArrayList arrayList = new ArrayList();
        List<TxnDetail> list = this.txnDetail;
        if (list != null && !list.isEmpty()) {
            for (TxnDetail txnDetail : this.txnDetail) {
                if (substring.equalsIgnoreCase("E23")) {
                    if (txnDetail.getNoReff().equalsIgnoreCase(str)) {
                        arrayList.add(txnDetail);
                    }
                } else if ((txnDetail.getType().equals("Transfer ke Bank Lain") && txnDetail.getFlagMultiKu().equals("Y")) || txnDetail.getType().equals("Pendebetan Dana")) {
                    arrayList.add(txnDetail);
                }
            }
        }
