...
/* loaded from: classes-dex2jar.jar:com/facebook/stetho/inspector/database/SqliteDatabaseDriver.class */
public class SqliteDatabaseDriver extends DatabaseDriver2<SqliteDatabaseDescriptor> {
    ...
    /* loaded from: classes-dex2jar.jar:com/facebook/stetho/inspector/database/SqliteDatabaseDriver$SqliteDatabaseDescriptor.class */
    ...
    public Database.ExecuteSQLResponse executeSQL(SqliteDatabaseDescriptor sqliteDatabaseDescriptor, String str, BaseDatabaseDriver.ExecuteResultHandler<Database.ExecuteSQLResponse> executeResultHandler) throws SQLiteException {
        ...
        try {
            ...
            return (c == 0 || c == 1) ? (Database.ExecuteSQLResponse) executeUpdateDelete(openDatabase, str, executeResultHandler) : c != 2 ? (c == 3 || c == 4 || c == 5) ? (Database.ExecuteSQLResponse) executeSelect(openDatabase, str, executeResultHandler) : (Database.ExecuteSQLResponse) executeRawQuery(openDatabase, str, executeResultHandler) : (Database.ExecuteSQLResponse) executeInsert(openDatabase, str, executeResultHandler);
        } finally {
            openDatabase.close();
        }
    }
